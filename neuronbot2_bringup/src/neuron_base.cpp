...
: Node("neuron_base", rclcpp::NodeOptions().use_intra_process_comms(true))
{
    ...
    std::string imu_repub_topic = declare_parameter("imu_repub_topic", "imu_data");
    std::string odom_sub_topic = declare_parameter("odom_sub_topic", "odom_data");
    ...
    raw_imu_sub = create_subscription<sensor_msgs::msg::Imu>(
       raw_imu_sub_topic , rclcpp::QoS(1), [=](sensor_msgs::msg::Imu::ConstSharedPtr msg) {on_raw_imu(msg); });
    ...
    imu_pub = create_publisher<sensor_msgs::msg::Imu>( imu_repub_topic, rclcpp::QoS(1));
